...
public class MarketDataFeedClient
{
    ...
    private readonly ILogger<MarketDataFeedClient> _logger;

    public MarketDataFeedClient(HttpClient http, string feedUrl, ILogger<MarketDataFeedClient> logger)
    {
        _http = http;
        _feedUrl = feedUrl;
        _logger = logger;
    ...
    public async Task<decimal?> GetBestPeakPriceAsync(CancellationToken cancellationToken = default)
    {
        ...
        {
            _logger.LogError(ex, "Market data feed call failed for {Endpoint}; returning fallback", endpoint);
            return null;
        }
    }
